		...
		try{
			...
			try {
				for (std::string line; std::getline(std::cin, line);){
					...
					if ((erreur.needToStopFd(parse->checkeur(line), parse->exit, nextLine, !fd.eof(), parse->getIndexLine()) == 1 ||  erreur.needToStopFd(parse->lexeur(line), parse->exit, nextLine, !fd.eof(), parse->getIndexLine()) == 1) && !fd.eof() != 0) {
						std::cout << line << "\n";
						return 0;
					}
				}
