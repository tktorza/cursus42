...
class CartController extends Controller
{
    ...
    public function renderAction(Restaurant $restaurant, Request $request)
    {
        ...
        return $this->render('ClabClickeatBundle:Cart:cart.html.twig', array(
            'cart' => $cart,
            'area' => $area,
        ));
    }
    ...
    public function addProductToCartAction(Request $request, Product $product)
    {
        ...
        if (!$request->isXmlHttpRequest()) {
            return $this->redirectToRoute('clab_call_center_order', array('slug' => $cart->getRestaurant()->getSlug()));
        } else {
            return $this->render('ClabCallCenterBundle:Default:cart.html.twig', array(
                'cart' => $cart,
     ...
     */
    public function updateQuantityAction(Request $request, Restaurant $restaurant, $hash, $add)
    {
    ...
    public function emptyAction(Request $request, Restaurant $restaurant)
    {
        ...
        if (!$request->isXmlHttpRequest()) {
            return $this->redirectToRoute('clab_call_center_order', array('slug' => $restaurant->getSlug()));
        } else {
            $cart = $this->get('app_shop.cart_manager')->getCart($restaurant);
    ...
    public function productOptionFormAction(Request $request, Product $product, $token = null, $slot = null)
    {
                    ...
                    if ($mealSlot['slot']->getId() == $slot) {
                        ...
                        foreach ($availableProducts as $availableProduct) {
                            if ($availableProduct['product'] == $product) {
                                ...
                                return $this->redirectToRoute('clab_call_center_meal_compose', array('token' => $token));
                            }
                        }
                    }
                }
        ...
        return $this->render('ClabCallCenterBundle:Order:productOptionForm.html.twig', $params);
    }
    ...
    public function mealComposeAction(Request $request, $token, $slot = null, $choice = null)
    {
        ...
        foreach ($mealData['slots'] as $key => $mealSlot) {
            if ((!$slot && !$mealSlot['product']) || $mealSlot['slot']->getId() == $slot) {
                ...
                return $this->render('ClabCallCenterBundle:Default:mealCompose.html.twig', array(
                    'meal' => $mealData,
                    'slot' => $mealSlot['slot'],
        ...
        if (!$request->isXmlHttpRequest()) {
            return $this->redirectToRoute('clab_call_center_order', array('slug' => $mealData['meal']->getRestaurant()->getSlug()));
        } else {
            $cart = $this->get('app_shop.cart_manager')->getCart($restaurant);
    ...
    public function couponAction(Request $request, Restaurant $restaurant)
    {
        ...
        $availabilityUser = $coupon->isAvailableForUser($customer);

        if ($availability && $availabilityUser) {
            $coupon->addUsedBy($customer);
        ...
        if (!$request->isXmlHttpRequest()) {
            ...
            return new JsonResponse([
                'success' => true,
                'cart' => $this->render('ClabCallCenterBundle:Default:cart.html.twig', array(
                    'cart' => $cart,
                    'area' => $area,
                    'restaurant' => $cart->getRestaurant(),
                    'noInteraction' => true
     ...
     * @ParamConverter("restaurant", class="ClabRestaurantBundle:Restaurant")
     */
    public function clearCartAction(Request $request, Restaurant $restaurant) {

        $this->get('app_shop.cart_manager')->clearCart($restaurant);
        $cart = $this->get('app_shop.cart_manager')->getCart($restaurant);
