...
;(function ($, window, document, undefined) {
	...
	var featuredset = [], locationset = [], normalset = [], markers = [], favorites = [];
	var filters = {}, locationData = {}, GeoCodeCalc = {}, mappingObj = {},map;
	...
	var defaults = {
		...
		'dragSearch'               : false,
		'infowindowTemplatePath'   : 'bundles/clabwhite/StoreLocatorTemplates/infowindow-description.html',
		'listTemplatePath'         : 'bundles/clabwhite/StoreLocatorTemplates/location-list-description.html',
		'KMLinfowindowTemplatePath': 'bundles/clabwhite/StoreLocatorTemplates/kml-infowindow-description.html',
		'KMLlistTemplatePath'      : 'bundles/clabwhite/StoreLocatorTemplates/kml-location-list-description.html',
		'listTemplateID'           : null,
		...
		// Language options
		'addressErrorAlert'        : 'Impossible de trouver l\'adresse',
		'autoGeocodeErrorAlert'    : 'La geolocation automatique a échoué. Veuillez l\'indiquer sur le formulaire ci dessus.',
		'distanceErrorAlert'       : 'Malheureusement le restaurant le plus proche est à plus de ',
		'mileLang'                 : 'mile',
		'milesLang'                : 'miles',
		'kilometerLang'            : 'kilometre',
		'kilometersLang'           : 'kilometres',
		'noResultsTitle'           : 'Nous n\'avons trouvé aucun résultat correspondant à vos critères, mais une petite modification pourrait suffire à donner des résultats. Voici quelques idées :',
		'noResultsDesc'            : '&nbsp;&nbsp;Retirez quelques filtres.<br>&nbsp;&nbsp;Étendez la zone de votre recherche.<br><br>Si vous lisez jusqu\'à là c\'est que nous n\'avons pas satisfait votre recherche. <br>Vous connaissez un restaurant dans cette zone et aimeriez le voir sur Clickeat ? Indiquez-le nous <a href="https://clickeat.typeform.com/to/S9gQo1">ici.</a>',
		'nextPage'                 : 'Suivant &raquo;',
		'prevPage'                 : '&laquo; Precedent',
		'favoritesJsonUrl'         : false
	};
	...
	$.extend(Plugin.prototype, {
		...
		init: function () {
			...
			Handlebars.registerHelper('niceDistance', function(distance) {
				if(distance){
					if(distance >=1){
						return distance+" km";
					}else{
		...
		_getData: function (lat, lng, address) {
			...
			// Remote data
				...
				$.ajax({
					...
					if(_this.settings.favoritesJsonUrl!==false){
						$.getJSON(_this.settings.favoritesJsonUrl).done(function(data){
							favorites = data;
							d.resolve(p);
		...
		_paginationOutput: function(currentPage, totalPages) {
			...
			// Previous page
			if( currentPage > 0 ) {
				output += '<li class="bh-sl-next-prev" style="color:#333;" data-page="' + prevPage + '" onclick="scrollToTop()">' + this.settings.prevPage + '</li>';
			}
			if (Math.ceil(totalPages) < 10) {
				...
				for (var p = 0; p < Math.ceil(totalPages); p++) {
					...
					if (p === currentPage) {
						output += '<li class="bh-sl-current" style="color:#e73162;" data-page="' + p + '" onclick="scrollToTop()">' + n + '</li>';
					}
					else {
						output += '<li data-page="' + p + '" style="color:#333;" onclick="scrollToTop()">' + n + '</li>';
					}
				}
			}else{
				var pages = [];
				for (var p = 0; p <10; p++) {
					var n = p + 1;
					if (p === currentPage) {
						pages[p] = '<li class="bh-sl-current" style="color:#e73162;" data-page="' + p + '" onclick="scrollToTop()">' + n + '</li>';
					}
					else {
						pages[p] = '<li data-page="' + p + '" style="color:#333;" onclick="scrollToTop()">' + n + '</li>';
					}
				}
				output+=pages.join(' ');
			}
			...
			// Next page
			if( nextPage < 10 && nextPage <= totalPages) {
				output += '<li class="bh-sl-next-prev" style="color:#333;" data-page="' + nextPage + '" onclick="scrollToTop()">' + this.settings.nextPage + '</li>';
			}
		...
		createMarker: function (point, name, address, letter, map, category) {
			...
			var  p = new google.maps.Point(12, 36);
		 ...
		 * @param map {Object} Google map
		 */
		checkVisibleMarkers: function(markers, map, storeStart, page) {
			...
			$(markers).each(function(x, marker){
				if(map.getBounds().contains(marker.getPosition())) {
					// Define the location data
					_this.listSetup(marker, storeStart, page);
		...
		processData: function (mappingObject, originPoint, data, page) {
			...
			var value;
			for(var loopcount=0;loopcount<locationset.length;loopcount++) {
				for (var key in locationset[loopcount]) {
					if (locationset[loopcount].hasOwnProperty(key)) {
						value = locationset[loopcount][key];
						if(key === 'openings'){
							...
							for(var i=0;i< value.length;i++) {
								if(now>=value[i]['start'] && now < value[i]['end'] ) {
									value = "true";
									break;
								}
							}
						...
						locationset[loopcount][key] = value;
					}
				}
			}
			if(_this.settings.favoritesJsonUrl!==false){
				for(var loopcount=0;loopcount<locationset.length;loopcount++) {
					if(favorites.indexOf(locationset[loopcount]['slug'])>=0){
						locationset[loopcount]['favorite'] = "true";
					}else{
						locationset[loopcount]['favorite'] = "false";
					}
				}
			}
			...
			if(_this.settings.visibleMarkersList === true) {
				...
				google.maps.event.addListenerOnce(map, 'idle', function(){
					_this.checkVisibleMarkers(markers, map, storeStart, page);
				});
